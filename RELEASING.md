...
```toml
[project]
version = "0.3.8"        # was 0.3.7
```

Use [semantic versioning](https://semver.org/): patch (`0.3.8`) for fixes, minor
...
git tag -a v0.3.8 -m "CableProbe 0.3.8"
git push origin v0.3.8

# create the GitHub Release -> triggers the publish workflow
gh release create v0.3.8 --title "v0.3.8" --notes "What changed in this release."
```

No `gh`? Do the release by hand at
`https://github.com/rosscooney/CableProbe/releases/new?tag=v0.3.8` — set the
...
```bash
pipx install "cableprobe==0.3.8"     # or: pip install --upgrade cableprobe
cableprobe --version
```
...
(`git push origin :refs/tags/v0.3.8` and delete it on GitHub), fix the problem,
