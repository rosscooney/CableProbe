...
```toml
[project]
version = "0.3.4"        # was 0.3.3
```

Use [semantic versioning](https://semver.org/): patch (`0.3.4`) for fixes, minor
...
git checkout -b release-0.3.4
git commit -am "Bump version to 0.3.4"
...
git tag -a v0.3.4 -m "CableProbe 0.3.4"
git push origin v0.3.4

# create the GitHub Release -> triggers the publish workflow
gh release create v0.3.4 --title "v0.3.4" --notes "What changed in this release."
```

No `gh`? Do the release by hand at
`https://github.com/rosscooney/CableProbe/releases/new?tag=v0.3.4` — set the
...
```bash
pipx install "cableprobe==0.3.4"     # or: pip install --upgrade cableprobe
cableprobe --version
```
...
(`git push origin :refs/tags/v0.3.4` and delete it on GitHub), fix the problem,
