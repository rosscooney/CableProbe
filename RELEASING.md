...
```toml
[project]
version = "0.4.9"        # was 0.4.8
```

Use [semantic versioning](https://semver.org/): patch (`0.4.9`) for fixes, minor
...
`## [0.4.9] - <date>` section in [`CHANGELOG.md`](CHANGELOG.md), and add the
matching `[0.4.9]: …/compare/v0.4.8...v0.4.9` link at the bottom. The GitHub
release notes are the same content.

```bash
git checkout main && git pull
git checkout -b release-0.4.9
git commit -am "Bump version to 0.4.9, update changelog"
git push -u origin release-0.4.9
...
git tag -a v0.4.9 -m "CableProbe 0.4.9"
git push origin v0.4.9

# create the GitHub Release -> triggers the publish workflow
gh release create v0.4.9 --title "v0.4.9" --notes "What changed in this release."
```

No `gh`? Do the release by hand at
`https://github.com/rosscooney/CableProbe/releases/new?tag=v0.4.9` — set the
...
```bash
pipx install "cableprobe==0.4.9"     # or: pip install --upgrade cableprobe
cableprobe --version
```
...
(`git push origin :refs/tags/v0.4.9` and delete it on GitHub), fix the problem,
