...
```toml
[project]
version = "0.3.10"        # was 0.3.9
```

Use [semantic versioning](https://semver.org/): patch (`0.3.10`) for fixes, minor
...
`## [0.3.10] - <date>` section in [`CHANGELOG.md`](CHANGELOG.md), and add the
matching `[0.3.10]: …/compare/v0.3.9...v0.3.10` link at the bottom. The GitHub
release notes are the same content.

```bash
git checkout main && git pull
git checkout -b release-0.3.10
git commit -am "Bump version to 0.3.10, update changelog"
git push -u origin release-0.3.10
...
git tag -a v0.3.10 -m "CableProbe 0.3.10"
git push origin v0.3.10

# create the GitHub Release -> triggers the publish workflow
gh release create v0.3.10 --title "v0.3.10" --notes "What changed in this release."
```

No `gh`? Do the release by hand at
`https://github.com/rosscooney/CableProbe/releases/new?tag=v0.3.10` — set the
...
```bash
pipx install "cableprobe==0.3.10"     # or: pip install --upgrade cableprobe
cableprobe --version
```
...
(`git push origin :refs/tags/v0.3.10` and delete it on GitHub), fix the problem,
