...
```toml
[project]
version = "0.3.9"        # was 0.3.8
```

Use [semantic versioning](https://semver.org/): patch (`0.3.9`) for fixes, minor
...
`## [0.3.9] - <date>` section in [`CHANGELOG.md`](CHANGELOG.md), and add the
matching `[0.3.9]: …/compare/v0.3.8...v0.3.9` link at the bottom. The GitHub
release notes are the same content.

```bash
git checkout main && git pull
git checkout -b release-0.3.9
git commit -am "Bump version to 0.3.9, update changelog"
...
git tag -a v0.3.9 -m "CableProbe 0.3.9"
git push origin v0.3.9

# create the GitHub Release -> triggers the publish workflow
gh release create v0.3.9 --title "v0.3.9" --notes "What changed in this release."
```

No `gh`? Do the release by hand at
`https://github.com/rosscooney/CableProbe/releases/new?tag=v0.3.9` — set the
...
```bash
pipx install "cableprobe==0.3.9"     # or: pip install --upgrade cableprobe
cableprobe --version
```
...
(`git push origin :refs/tags/v0.3.9` and delete it on GitHub), fix the problem,
