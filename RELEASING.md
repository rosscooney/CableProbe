...
```toml
[project]
version = "0.4.5"        # was 0.4.4
```

Use [semantic versioning](https://semver.org/): patch (`0.4.5`) for fixes, minor
...
`## [0.4.5] - <date>` section in [`CHANGELOG.md`](CHANGELOG.md), and add the
matching `[0.4.5]: …/compare/v0.4.4...v0.4.5` link at the bottom. The GitHub
release notes are the same content.

```bash
git checkout main && git pull
git checkout -b release-0.4.5
git commit -am "Bump version to 0.4.5, update changelog"
git push -u origin release-0.4.5
...
git tag -a v0.4.5 -m "CableProbe 0.4.5"
git push origin v0.4.5

# create the GitHub Release -> triggers the publish workflow
gh release create v0.4.5 --title "v0.4.5" --notes "What changed in this release."
```

No `gh`? Do the release by hand at
`https://github.com/rosscooney/CableProbe/releases/new?tag=v0.4.5` — set the
...
```bash
pipx install "cableprobe==0.4.5"     # or: pip install --upgrade cableprobe
cableprobe --version
```
...
(`git push origin :refs/tags/v0.4.5` and delete it on GitHub), fix the problem,
